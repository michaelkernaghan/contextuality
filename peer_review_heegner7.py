"""Send heegner7_letter.tex to GPT-5.2 for peer review."""
...
# Read the paper
tex_path = Path("paper/heegner7_letter.tex")
...
prompt = f"""You are a referee for Physical Review Letters. Please provide a detailed peer review of the following manuscript. The paper reports new Kochen-Specker sets from algebraic number fields and a mechanism (auxiliary rays) that controls contextual advantage strength.

Evaluate:
1. **Academic Rigor**: Are claims properly supported? Is the methodology sound? Are there logical gaps?
2. **Novelty**: Are the Heegner-7 and golden ratio KS sets genuinely new contributions? Is the auxiliary ray mechanism a new insight?
...
Note: The author (Michael Kernaghan) published the 20-vector KS result in 1994 (J. Phys. A) and the 36-vector 8-dimensional result with Peres in 1995 (Phys. Lett. A), so has a track record in this area.
...
review_path = review_dir / f"heegner7_letter-review-{timestamp}.txt"

header = f"""Peer Review: New Kochen-Specker sets from algebraic number fields with enhanced contextual advantage
