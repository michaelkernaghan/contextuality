...
\begin{abstract}
\noindent
We prove that for a two-element coordinate alphabet
$\mathcal{A} = \{0, \pm 1, \pm x\}$, where~$x$ is a ring-of-integers
generator of a quadratic extension $K/\Q$ (or $x=2$), the \emph{raw}
ray set $S(\mathcal{A}) \subset \C^3$ (before cross-product completion)
is KS-uncolorable if and only if
\begin{enumerate}
\item[(i)] $|x|^2 = 2$ or $x = 2$ \emph{(modulus-2)}, or
\item[(ii)] $\Norm{x} = 1$ and $\Tr{x} = -1$ \emph{(phase)}.
\end{enumerate}
Sufficiency follows from known KS constructions; necessity is proved
analytically for all quadratic generators except the golden ratio
(which is treated separately),
via a finite enumeration of three-term vanishing sums from the product
set $\bar{\mathcal{A}} \cdot \mathcal{A}$ and a cross-product closure
argument showing that, when neither condition holds, the orthogonality
graph decomposes into independently colorable subproblems admitting an
explicit $\{0,1\}$-coloring.  As
corollaries we recover a Heegner-number characterization of the
KS-contextual quadratic alphabets.  The classification
connects to the result of Cortez, Schmid, and Spekkens that $\Z[1/6]$ is
the minimal ring supporting KS contextuality in dimension~3.
\end{abstract}
...
algebraic integer generating the ring of integers of a quadratic
extension $K/\Q$, or $x = 2$ (the degenerate rational case $K = \Q$).
The ray set $S(\mathcal{A}) \subset \C^3$ is KS-uncolorable if and only
if one of:
\begin{enumerate}
  \item[(i)] $|x|^2 = 2$ (equivalently, $|\Norm{x}| = 2$ for imaginary
    quadratic~$x$), or $x = 2$ \emph{(modulus-2)};
  \item[(ii)] $\Norm{x} = 1$ and $\Tr{x} = -1$ \emph{(phase)}.
\end{enumerate}
\end{theorem}

The proof occupies Section~\ref{sec:proof}.  Section~\ref{sec:consequences}
derives corollaries including the Heegner characterization and the
Galois symmetry of the orthogonality graph.  Section~\ref{sec:discussion} discusses
connections to the Cortez--Schmid--Spekkens ring-theoretic result.
...
\begin{theorem}[Galois classification]\label{thm:galois}
Let $\mathcal{A} = \{0,\pm 1,\pm x\}$ with $x \notin \{0,\pm 1\}$ an
algebraic integer generating $\mathcal{O}_K$ for a quadratic extension
$K/\Q$, or $x = 2$ (rational case).  The ray set
$S(\mathcal{A}) \subset \C^3$ is KS-uncolorable if and only if
\begin{enumerate}
  \item[\emph{(i)}] $|x|^2 = 2$, or $x = 2$ \emph{(modulus-2)}, or
  \item[\emph{(ii)}] $\Norm{x} = 1$ and $\Tr{x} = -1$ \emph{(phase)}.
\end{enumerate}
\end{theorem}

\noindent\textit{Sufficiency.}
Each case is witnessed by an explicit KS construction.  The rational
case $x = 2$ gives the 31-vector CK-31 set (Conway--Kochen, rigidity
proved by Trandafir--Cabello~\cite{TrandafirCabello2025}).
Condition~(i) with $x = \sqrt{2}$ gives the 33-vector Peres
set~\cite{Peres1991}; with $x = \sqrt{-2}$ the 33-vector
$\Z[\sqrt{-2}]$ set; with $x = 1{+}i$ ($|x|^2 = 2$) the 33-vector
Gaussian set (graph-isomorphic to
Peres~\cite{Kernaghan2026islands}); and with
$x = \tfrac{1+\sqrt{-7}}{2}$ ($|x|^2 = 2$) the 43-vector Heegner-7
set~\cite{Kernaghan2026islands}.  Condition~(ii) is realized by
$x = \omega = e^{2\pi i/3} = (-1{+}\sqrt{-3})/2$, the alternative
Eisenstein generator with $\Norm{\omega} = 1$ and
$\Tr{\omega} = -1$, yielding the 33-vector Cabello
set~\cite{Cabello2025simplest}.  \hfill$\square$
...
Suppose $x$ satisfies neither condition~\emph{(i)} nor~\emph{(ii)} of
Theorem~\ref{thm:galois}.  Then the ray set $S(\mathcal{A})$ admits an
explicit valid $\{0,1\}$-coloring.
\end{lemma}

\noindent\textit{Remark.}
By Lemma~\ref{lem:vanishing}, when neither~(i) nor~(ii) holds,
Rows~1--3a and~5--6 of Table~\ref{tab:vanishing} are excluded.
Row~3b ($\Tr{x} = 1$) survives for imaginary quadratic generators
with $d \equiv 1 \pmod{4}$, and Row~4 ($|x|^2 = 1 + x$, realized
uniquely by $x = \varphi = (1{+}\sqrt{5})/2$) is a real quadratic
case treated separately in Section~\ref{sec:discussion}.
The proof below handles the Row~3b vanishing sum explicitly, showing
that the all-nonzero triads it permits do not obstruct colorability.

\begin{proof}
By Lemma~\ref{lem:vanishing}, Rows~1--3a and~5--6 are excluded by the
negation of~(i) and~(ii).  We exclude the golden ratio
$x = \varphi$ (Row~4, addressed in Section~\ref{sec:discussion}).
For the remaining generators, the only surviving vanishing sum is
Row~3b: $1 - x - \bar{x} = 0$, which holds precisely when $\Tr{x} = 1$
(imaginary quadratic, $d \equiv 1 \pmod{4}$).  When $\Tr{x} \neq 1$,
no vanishing sum exists at all.  The proof proceeds in three steps.
...
is represented by the \emph{Hermitian cross product}
\[
  u_k = \bar{v}_{k+1} w_{k+2} - \bar{v}_{k+2} w_{k+1}
  \quad (\text{indices mod } 3),
\]
which satisfies $\braket{u}{v} = \braket{u}{w} = 0$ by construction.
(For real fields, $\bar{v}_k = v_k$ and this reduces to the standard
bilinear cross product.)  We show that $u \notin \mathcal{A}^3$ up to
projective equivalence.

With $w_3 = 0$, the Hermitian cross product gives
$u = (-\bar{v}_3 w_2,\; \bar{v}_3 w_1,\; \bar{v}_1 w_2 - \bar{v}_2
w_1)$.  The first two components lie in
$\bar{\mathcal{A}} \cdot \mathcal{A}$; the third is a difference of
two such products.
...
\begin{corollary}[Heegner characterization]\label{cor:heegner}
Among imaginary quadratic fields $\Q(\sqrt{-d}\,)$, there exists
a generator~$y$ of~$\mathcal{O}_K$ such that
$\{0, \pm 1, \pm y\}$ is KS-uncolorable if and only if
$d \in \{1, 2, 3, 7\}$, realized by $y = 1{+}i$,\;
$\sqrt{-2}$,\; $\omega$,\; $(1{+}\sqrt{-7})/2$
respectively.
\end{corollary}

\begin{proof}
Every generator of $\mathcal{O}_K$ as a $\Z$-algebra has the form
$y = a + \varepsilon x_0$ where $x_0$ is the canonical generator,
$\varepsilon = \pm 1$, and $a \in \Z$.  (Proof: $\{1, x_0\}$ is a
$\Z$-basis of $\mathcal{O}_K$, so $y = a + bx_0$ with $a, b \in \Z$;
the condition $\Z[y] = \mathcal{O}_K$ requires that $\{1, y\}$ also
be a $\Z$-basis, which forces $b = \pm 1$.)  The Hermitian squared modulus
is $|y|^2 = a^2 + \varepsilon a \, \Tr{x_0} + |x_0|^2$, a quadratic
in~$a$ with minimum value $|x_0|^2 - \Tr{x_0}^2/4$.  Condition~(i)
requires $|y|^2 = 2$; condition~(ii) requires $\Norm{y} = 1$ and
$\Tr{y} = 2a + \varepsilon \Tr{x_0} = -1$.

\begin{itemize}
\item $d = 1$: $x_0 = i$, $|x_0|^2 = 1$.  Then
  $|y|^2 = a^2 + 1 \geq 1$; $|y|^2 = 2$ iff $a = \pm 1$, giving
  $y = \pm 1 + i$, but $\{0, \pm 1, \pm(1{+}i)\}$ has $|1{+}i|^2 = 2$:
  condition~(i) with the Gaussian generator.
  \emph{KS-uncolorable} (Gaussian island, graph-isomorphic to Peres).
\item $d = 2$: $x_0 = \sqrt{-2}$, $|x_0|^2 = 2$.
  Condition~(i) with $a = 0$. \emph{KS-uncolorable.}
\item $d = 3$: $x_0 = (1{+}\sqrt{-3})/2$, $|x_0|^2 = 1$,
  $\Tr{x_0} = 1$.  For $\varepsilon = +1$: $|y|^2 = a^2 + a + 1$;
  minimum $3/4$, never equals~$2$ for integer~$a$.
  For $\varepsilon = -1$: $|y|^2 = a^2 - a + 1$; minimum $3/4$,
  also never~$2$.  So condition~(i) fails for all generators.
  For~(ii): $\Tr{y} = 2a + \varepsilon = -1$.
  With $\varepsilon = +1$: $a = -1$, giving $y = -1 + x_0 =
  (-1{+}\sqrt{-3})/2 = \omega$ with $\Norm{\omega} = 1$. \checkmark\
  With $\varepsilon = -1$: $a = 0$, giving $y = -x_0 =
  (-1{-}\sqrt{-3})/2 = \bar\omega$ with $\Norm{\bar\omega} = 1$
  and $\Tr{\bar\omega} = -1$. \checkmark\
  Condition~(ii). \emph{KS-uncolorable.}
\item $d = 7$: $x_0 = (1{+}\sqrt{-7})/2$, $|x_0|^2 = 2$.
  Condition~(i) with $a = 0$. \emph{KS-uncolorable.}
\item $d = 11$: $|x_0|^2 = 3$, $\Tr{x_0} = 1$.
  For $\varepsilon = +1$: $|y|^2 = a^2 + a + 3$; minimum $11/4 > 2$.
  For $\varepsilon = -1$: $|y|^2 = a^2 - a + 3$; minimum $11/4 > 2$.
  So (i) fails.  For~(ii) with $\varepsilon = +1$: $\Tr{y} = 2a + 1
  = -1$ gives $a = -1$, $\Norm{y} = 1 - 1 + 3 = 3 \neq 1$.
  With $\varepsilon = -1$: $\Tr{y} = 2a - 1 = -1$ gives $a = 0$,
  $\Norm{y} = 0 - 0 + 3 = 3 \neq 1$.  Neither.
\item $d \geq 19$: $|x_0|^2 \geq 5$.  The minimum of
  $|y|^2 = a^2 + a + |x_0|^2$ is $|x_0|^2 - 1/4 > 2$, so (i) fails
  for all~$a$.  For~(ii): $\Norm{y} \geq |x_0|^2 - 1/4 > 1$ for
  $|x_0|^2 \geq 2$.  Neither.
\end{itemize}
\end{proof}
...
Cortez--Schmid--Spekkens result~\cite{CortezMoralesReyes2022} that
...
\textit{Minimal algebraic contextuality via partial rings},
Phys.~Rev.~Lett.\ \textbf{129}, 230401 (2022).
