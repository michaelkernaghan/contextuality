...
The Peres and $\Z[\sqrt{-2}]$ islands produce graph-isomorphic minimal sets (verified by VF2), yielding five distinct graph types. Non-isomorphism among the remaining pairs is established by differing degree sequences. The Heegner-7 and golden ratio islands are new constructions not found in the four previously known algebraic KS families; details are reported in a companion paper~\cite{Kernaghan2026heegner}.
...
\emph{Important caveat}: the rational alphabet is projectively equivalent to the integer alphabet (the identical 109-ray count confirms this), and the mixed alphabet contains the integer alphabet as a subset. We cannot rule out that the minimization procedure simply rediscovers the embedded CK configuration in each case. This observation is therefore consistent with---but does not prove---a uniqueness claim.
...
M.~Kernaghan,
``New Kochen--Specker sets from algebraic number fields: the Heegner-7 and golden ratio constructions'' (in preparation).
