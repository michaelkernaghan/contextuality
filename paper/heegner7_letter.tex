...
The Heegner-7 full pool achieves $\vartheta/\alpha = 1.118$---a 12\% quantum advantage, the largest among the five algebraic KS ray pools with computed CSW data.  (Numerically, this ratio coincides with $\sqrt{5}/2 \approx 1.118$, the quantum-to-classical ratio of the KCBS pentagon---the fundamental contextuality scenario~\cite{CSW2014}; whether this coincidence has structural significance is an open question.) For comparison, CK-31's minimized set achieves only $\vartheta/\alpha = 1.065$, and most minimized KS sets are $\vartheta$-perfect ($\vartheta = \alpha$). Yet the Heegner-7 minimized 43-vector set itself satisfies $\vartheta(G) = \alpha(G)$, exhibiting no violation at all.
