...
The classical KS constructions in $\R^3$ use rays whose coordinates are small integers: Conway and Kochen's 31-vector set~\cite{ConwayKochen} uses directions $(a,b,c)$ with $a,b,c \in \{0, \pm 1, \pm 2\}$---geometrically, these point toward vertices, edge midpoints, and face centers of nested cubes. Orthogonality between two such rays reduces to an integer dot product equaling zero: $a_1 a_2 + b_1 b_2 + c_1 c_2 = 0$. The key observation is that the set of allowed coordinate values---the \emph{alphabet}---determines which orthogonalities are available, and hence which KS sets can be constructed. Different algebraic number rings provide different alphabets.

Concretely, given an alphabet $\mathcal{A} \subset \C$ (a finite set of allowed coordinates drawn from a number ring), we generate all rays $[v] \in \C P^2$ whose homogeneous coordinates lie in $\mathcal{A}$, identify orthogonal pairs via $\langle u | v \rangle = 0$, and collect orthonormal bases (triples of mutually orthogonal rays). KS-uncolorability---the nonexistence of a consistent $\{0,1\}$-valued coloring assigning exactly one~1 per basis---is decided by SAT encoding: each ray $i$ gets a Boolean variable $x_i$; each basis $\{a,b,c\}$ contributes clauses enforcing $x_a + x_b + x_c = 1$; each orthogonal pair $(i,j)$ contributes $\neg x_i \vee \neg x_j$. The encoding is solved by Glucose4~\cite{AudemardSimon2018} via PySAT. We minimize by randomized greedy deletion (500 trials, random seed 42), which yields a small KS subset but does not guarantee minimality.
...
Both constructions arise from a pattern observed across all tested number fields: when the generator norm $|\alpha|^2 \leq 2$, the ring of integers supports enough exact orthogonality cancellations to produce KS sets; when $|\alpha|^2 > 2$, it does not. The integer identity $1{+}1=2$, the Eisenstein identity $1{+}\omega{+}\omega^2 = 0$, and the Heegner-7 identity $\alpha\bar\alpha = 2$ are different manifestations of this single constraint. We have verified this pattern computationally across 40+ quadratic and cyclotomic fields (all nine imaginary quadratic fields of class number~1, real quadratic fields $\Q(\sqrt{d})$ for $d = 2, \ldots, 30$, and roots of unity of order $n \leq 30$); the full algebraic classification is reported separately~\cite{Kernaghan2026landscape}.
