...
We also tested whether CK-31 can be compressed by \emph{merging} non-orthogonal ray pairs (identifying two vertices) while preserving KS-uncolorability. Of the $\binom{31}{2} - 67 = 398$ non-orthogonal pairs, exactly 15 merges preserve abstract KS-uncolorability, producing 30-vertex graphs with 17--19 triads and 66--67 pairs. Z3~\cite{Z3} with nonlinear real arithmetic could not resolve their $\R^3$ realizability (``unknown'' after 60\,s per instance, 90 variables each).

To circumvent this, we reformulated realizability within the integer pool as a \emph{finite constraint satisfaction problem}: each of 30 vertices must be assigned one of the 49 (canonicalized) integer rays from $\{0,\pm1,\pm2\}$, with orthogonality edges requiring perpendicular rays and non-edges requiring non-perpendicular rays. This encodes as a SAT instance with $30 \times 49 = 1470$ Boolean variables plus cardinality and compatibility constraints, solvable in under 1\,s per graph. All 15 merged graphs are \emph{unrealizable} within the integer pool. Combined with the Z3 results, this means the 15 abstract 30-vertex KS graphs cannot be realized using any known algebraic construction; their $\R^3$ realizability remains the fundamental open question.
...
The vertex-merging experiment reveals the nature of the obstacle: abstract KS-uncolorable graphs on 30~vertices exist (15 were found from CK-31 alone), but they are unrealizable within the integer pool and their general $\R^3$ realizability exceeds current SMT capabilities. This confirms that the 24--31 gap is fundamentally a realizability problem, not a combinatorial one.

We do not prove that 31 is optimal. A CEGAR (counterexample-guided abstraction refinement) search for 30-ray KS subsets of the integer pool eliminated 500,000 candidate subsets without finding one, consistent with CK-31's optimality within $\{0,\pm1,\pm2\}$, but the search space ($\binom{62}{30} \approx 10^{17}$) was not fully exhausted. Combined with the greedy minimization (200 trials, all converging to 31), integer-pool CSP (all 15 merged graphs unrealizable), and deep criticality ($k \leq 8$ exhaustive), the evidence strongly suggests that 31 is optimal within integer coordinates. A sub-31 KS set, if it exists, would require rays outside the $\{0,\pm1,\pm2\}$ alphabet---a fundamentally different construction method.
