...
\textbf{Numerical optimization (Strategy 4).} All four non-algebraic methods fail: simulated annealing, CK-31 perturbation, orthogonal completion, and soft-tolerance annealing each produce at most 5--6 orthogonal pairs and zero triads.  Exact orthogonality is measure-zero in continuous space; these failures motivate the algebraic approach.

\textbf{Vertex merging.} We tested whether minimal KS sets can be compressed by merging non-orthogonal ray pairs (identifying two vertices into one that inherits both neighborhoods).  Across all six islands, every non-orthogonal pair merge preserves KS-uncolorability: 3{,}756 merges total with 100\% preservation.  We conjecture that every minimal KS set is \emph{merge-saturated}.  For CK-31's 394 merges (each a 30-vertex abstract KS graph), Z3~\cite{Z3} returns ``unknown'' on $\R^3$ realizability.  Reformulating as a finite SAT encoding (Boolean assignment of vertices to the 49 pool rays, with orthogonality and injectivity constraints) resolves all 394 instances in under 1\,s each: all are \emph{unrealizable} within the integer pool.  The 24--31 gap is thus a realizability problem: combinatorially viable 30-vertex KS graphs exist, but none embed in the known algebraic pools.
