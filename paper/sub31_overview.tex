...
with fewer than~31 vertices, but none is realizable in~$\R^3$;
a complementary vertex-merging analysis produces 394 combinatorially valid
30-vertex KS graphs, also unrealizable.
...
We establish an empirical \emph{modulus-2 boundary}---all tested alphabets
lacking the coordinate~$\pm2$ fail to produce KS-uncolorable pools---and
...
identified in our companion paper \cite{Kernaghan2026algebraic}, we apply minimum
unsatisfiable subset (MUS) extraction and an OCUS (optimal constrained unsatisfiable
subset) procedure to certify sharp minimum KS sizes: 31 rays for the integer
alphabet $\{0,\pm 1,\pm 2\}$, 33 rays for the Peres, Eisenstein, and
$\Z[\sqrt{-2}]$ alphabets, 43 rays for the Heegner-7 alphabet, and 52 rays for
the golden-ratio alphabet. The OCUS result for the 49-ray integer pool establishes
that~31 is the exact minimum within that pool, not merely an upper bound.

Second, we identify what we call the \emph{modulus-2 boundary}: every tested
coordinate alphabet that lacks the entry $\pm 2$ fails to produce a
KS-uncolorable ray pool, while every alphabet containing $\pm 2$ succeeds.
This is consistent with the hypothesis that the cancellation identity $1 + 1 = 2$
is not merely sufficient but necessary for KS-uncolorability in the integer setting.
In a companion paper~\cite{Kernaghan2026algebraic}, we established that
KS-uncolorability in dimension~3 requires one of two cancellation mechanisms,
of which integer arithmetic with~$\pm 2$ is the simplest and most constrained.

Third, a graph perturbation search over 500,000 random trials finds 61,702
abstract KS-uncolorable hypergraphs with fewer than~31 vertices, but none of
these abstract structures is realizable in $\R^3$. A complementary vertex-merging
analysis produces 394 distinct 30-vertex KS-uncolorable graphs derived from
CK-31 by identification of pairs of rays; again, none is geometrically realizable.
This dichotomy---abstract sub-31 KS candidates exist in abundance, yet $\R^3$
geometry prevents every one of them from being realized---identifies the
\emph{realizability barrier} as the core obstruction to a sub-31 KS set.
...
%%----------------------------------------------------------------------
\section{Exact minimization within algebraic pools}
\label{sec:exact}
%%----------------------------------------------------------------------
...
establish an empirical modulus-2 boundary: alphabets lacking the coordinate
$\pm2$ cannot produce KS sets in our survey. This is consistent with the
hypothesis that the $1+1=2$ cancellation identity is structurally necessary for
KS-uncolorability from integer-like alphabets in dimension~3, and extends to
several non-integer fields as well. We conclude: \textit{expanded coordinate
sets from all tested fields fail to produce sub-31 KS sets, establishing the
empirical modulus-2 boundary as a robust feature of the known algebraic
landscape.}
...
\textbf{The geometric requirement is the bottleneck.} Of the 61,702 abstract sub-31 candidates, not a single one admits a realization in~$\R^3$. The 1,904 near-realizable cases (residual $< 0.1$) are as close as the landscape permits, and the best achievable residual is $3.8\times 10^{-2}$---more than six orders of magnitude above the acceptance threshold.
...
\begin{remark}
The 61,702 abstract sub-31 KS-uncolorable hypergraphs are combinatorial objects: they exist as hypergraphs but not (so far as we can determine) as ray sets in~$\R^3$. Their existence shows that the KS theorem's combinatorial logic does not require 31 rays; the constraint comes from the interaction of that logic with three-dimensional Euclidean geometry.
\end{remark}
...
\item \emph{Vertex merging}: all 394 combinatorially valid 30-vertex KS graphs derived from CK-31 are unrealizable within the integer pool (finite pool SAT, all UNSAT).
\item \emph{Graph perturbation} (Section~\ref{sec:perturbation}): 61,702 abstract sub-31 KS-uncolorable hypergraphs are all unrealizable in~$\R^3$ (numerical optimization, zero residuals $< 10^{-10}$).
\end{enumerate}
In total, at least 62,096 distinct abstract sub-31 KS-uncolorable hypergraphs have been tested; none is realizable.
\end{observation}

The 24--31 gap is not a combinatorial gap. Abstract KS-uncolorable hypergraphs with $n < 31$ exist in abundance. The gap is a \emph{realizability} gap: three-dimensional real geometry refuses to accommodate them.
...
This is an $\R^3$-specific constraint: in higher dimensions, $v^\perp$ has dimension $\geq 2$ and orthogonal pairs need not form a matching. The 2-dimensionality of $v^\perp$ in $\R^3$ imposes rigid structure on triad sharing.

\begin{conjecture}[Plane matching as realizability obstruction]
\label{conj:planematching}
There exist abstract KS-uncolorable hypergraphs on $n < 31$ vertices whose local triad structure is compatible with the plane matching property, but for which no global consistent assignment of rays in~$\R^3$ exists.
\end{conjecture}
