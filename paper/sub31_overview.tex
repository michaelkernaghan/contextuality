...
is bounded between 24 (established 2023) and~31 (circa 1990), a gap whose
current width has persisted since the SAT-based lower-bound improvements of
Li--Bright--Ganesh and Kirchweger--Peitl--Szeider.
We report a systematic search for KS sets with fewer than~31 vectors using
eight complementary strategies: SAT-verified minimization within six algebraic
...
triad density thresholds, a graph perturbation search over 500,000 trials,
and vertex-merging realizability analysis.
...
finite SAT encoding.
The convergence of exact and numerical evidence identifies a
\emph{realizability barrier}: abstract sub-31 KS candidates exist in
abundance, but none has been realized as rays in $\R^3$.
...
finite set of rays in $\R^d$ (or $\C^d$; equivalently, unit vectors up to scalar multiples) together with
...
KS-colorable. They further conjectured, subject to two assumptions---that the minimum KS set is rigid and that it contains the minimal state-independent contextuality (SI-C) set---that $n_{\min}^{(3)} = 31$. This conjecture represents the sharpest current
...
$3.8 \times 10^{-2}$, more than eight orders of magnitude above the acceptance threshold).
...
genuinely KS-uncolorable.  The method does not rely on external solvers for
...
subject to the assumptions that the minimum KS set is rigid and contains the
minimal SI-C set, they
...
Our implementation iteratively enumerates hitting sets of all MUSes found so far,
using a SAT oracle to test each candidate for unsatisfiability, and advances the
cardinality bound until it certifies that no unsatisfiable subset of size $< k^*$
exists. This provides an exhaustive lower bound on the minimum
...
yielding a Jaccard similarity of approximately 0.71. CK-31 is one of the six.
All six share the same orthogonality graph (verified by VF2 isomorphism
testing~\cite{Kernaghan2026algebraic}), consistent with the rigidity
result of Trandafir and Cabello~\cite{TrandafirCabello2025}.
...
KS-uncolorable (--- means colorable); ``Best'' is the smallest KS subset
found by greedy minimization in 300--500 trials (--- means not applicable).}
\label{tab:alphabets}
\begin{tabular}{lrcl}
\toprule
Alphabet & Rays & Best & Note \\
\midrule
$\{0,\pm1,\pm2\}$ & 49 & 31 & Baseline (CK-31) \\
$\{0,\pm1,\pm2,\pm3\}$ & 145 & 31 & Absorbs to CK-31 \\
$\{0,\pm1,\pm2,\pm3,\pm4\}$ & 289 & 36$^\dagger$ & Heuristic limit \\
$\{0,\pm1,\pm3\}$ & 49 & --- & Not KS \\
$\{0,\pm1,\pm4\}$ & 49 & --- & Not KS \\
...
$^\dagger$Since this pool contains the $\{0,\pm1,\pm2\}$ sub-pool, its true
minimum is at most~31 (inherited from CK-31). The reported value of~36
reflects greedy heuristic incompleteness on the larger 289-ray search space,
not a structural property of the pool.
...
only minimum~36 under greedy minimization, though the true minimum is at
most~31 since this pool contains the $\{0,\pm1,\pm2\}$ sub-pool as a subset.
The failure to find~31 reflects the difficulty of greedy search in the larger
289-ray space, not a structural barrier imposed by the additional rays.
...
alphabets of the form $\{0,\pm 1,\pm x\}$, only those where the cancellation
identity $1 + 1 = 2$ is available---either because $|x|^2 = 2$ or because
$x = 2$ itself (the degenerate integer case with $|x|^2 = 4$)---produce
KS-uncolorable pools. (Non-integer fields such as the Eisenstein
...
\textbf{Key insight: exact orthogonality is measure-zero.} In the space of unit vectors in $\R^3$ or $\C^3$, the condition $\langle u, v \rangle = 0$ defines a measure-zero set. For the unguided random-start methods tested here, there is no gradient signal connecting near-orthogonal configurations to exact ones: the ``number of exact orthogonal pairs'' objective is constant (zero) almost everywhere, jumping discontinuously on a set of measure zero. More sophisticated continuous methods (homotopy continuation, manifold-constrained search) are not ruled out in principle, but the absence of any gradient signal in the tested landscape---and the complete evaporation of near-orthogonal structure under tolerance tightening---suggests that algebraic structure is necessary for producing the exact orthogonalities that KS sets require.
...
A KS set $S$ is \emph{deletion-minimal} if removing any single ray from $S$ produces a colorable set. Deletion-minimality implies that $S$ has no proper KS subset: every proper subset is contained in some single-deletion subset, and every single-deletion subset is colorable; since colorability is hereditary (any subset of a colorable set is colorable), every proper subset is colorable.
...
Total: 11{,}460{,}948 exhaustive SAT checks plus $4 \times 10^6$ sampled checks. \emph{All subsets are colorable.}
...
\end{enumerate}
In total, 62,096 abstract sub-31 KS-uncolorable hypergraphs have been tested (394 from merging, 61,702 from perturbation; overlap and isomorphism reduction not yet performed). No realization has been found by any method.
\end{observation}
...
We distinguish this \emph{structural} realizability barrier from the \emph{computational} realizability bottleneck identified by Li, Bright, and Ganesh~\cite{LiBrightGanesh2024}, whose pipeline stalls because Z3 returns ``unknown'' on embeddability queries above order~$\sim$20. Their observation is that realizability \emph{checking} is hard; ours is that realizability itself appears to \emph{fail} for sub-31 candidates---62,096 tested, zero realized. The two findings are complementary: theirs explains why the lower bound has stalled at~24; ours suggests that closing the gap from above may require proving a geometric impossibility rather than finding a construction.
...
The computational evidence is strong: no sub-31 KS set has been found by any of eight independent methods, and the realizability barrier (Observation~\ref{obs:barrier}) shows that abstract sub-31 KS hypergraphs are combinatorially plentiful but have resisted all attempts at geometric realization. We frame the following as structural insights and candidate proof directions, not as partial proofs.
...
annealing, perturbation, and graph perturbation). Two additional approaches
belong to this family but were not tested systematically: polytope-based
generation from regular solids (tested for higher dimensions by
Pavicic et al.~\cite{Pavicic2004} but less natural in $d = 3$ where no
regular polytope produces KS sets), and homotopy continuation methods
(noted by Pavicic et al.\ as having been ``tested without success on
representative systems''). Heuristic methods explore
...
Cortez, Morales, and Reyes introduced the invariant $N(S) = \mathrm{lcm}\{\|v\|^2
: v \in S\}$ for a KS set $S$ with integer coordinates, and proved that $S$
must be defined over the ring $\Z[1/N(S)]$. (Their result is formulated in
terms of \emph{algebraic hidden states}---the non-existence of morphisms from
partial rings of symmetric matrices---which is a ring-theoretic formulation
of contextuality related to but distinct from KS-uncolorability of ray pools.) For $\CK$, $N = 30$, corresponding
...
We have described eight complementary strategies---algebraic pool construction
...
\item \textbf{Does merge saturation preserve minimality?}
Proposition~\ref{prop:merge} proves that merging non-adjacent vertices preserves
KS-uncolorability. A stronger open question is whether the \emph{merged set is
itself minimal} (deletion-minimal): does merging a minimal KS set always produce
another minimal KS set? The 100\% preservation of uncolorability across 3,756
merges over six islands follows from the proposition; whether the merged sets are
also deletion-minimal, and whether this property characterizes the space of
minimal KS sets, remains open. \textit{(Open; the uncolorability preservation
is proved, but minimality preservation is not.)}
...
This paper was developed using an LLM-assisted workflow in which all computations, search strategies, and manuscript text were generated by a large language model (Claude, Anthropic) under the author's direction.  To mitigate known failure modes of LLM-generated scientific work---particularly silent fabrication and pressure-induced confabulation~\cite{Schwartz2026}---three verification layers were applied: (1)~all computational claims are independently reproducible via the scripts in the companion repository with deterministic seeding (\texttt{random.seed(42)}); (2)~key results were cross-verified using a competing LLM (GPT, OpenAI) through structured adversarial peer review; (3)~all claims about published results were verified by direct reading of the cited papers.  All SAT-based claims (uncolorability, OCUS certification, criticality) are machine-checkable proofs within the PySAT framework.
...
Claude (Anthropic) performed all computational work including Python script development, SAT-based search and verification, numerical optimization, graph perturbation analysis, and manuscript preparation; cross-verification was performed using GPT (OpenAI) through structured adversarial peer review.  The author directed the research program, selected strategies, validated results against the published literature, and accepts full responsibility for the scientific content and integrity of this paper.
...
arXiv:2601.02484 [physics.gen-ph] (2026).
