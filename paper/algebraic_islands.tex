...
\begin{abstract}
We show computationally that Kochen--Specker (KS) uncolorability in three-dimensional Hilbert space is controlled by a simple algebraic invariant: the squared norm of the ring-of-integers generator. Surveying quadratic fields, cyclotomic fields, and golden ratio extensions, we find that KS sets exist only when this norm is at most 2, explaining why constructions cluster into six discrete algebraic islands. Three are new: the Heegner-7 ring $\Z[(1+\sqrt{-7})/2]$ (43 vectors, a genuinely new orthogonality graph), the golden ratio field $\Q(\varphi)$ (52 vectors, revealed only by cross-product completion), and $\Z[\sqrt{-2}]$ (a new algebraic realization of the Peres-type structure). Different islands yield different operational costs: using SAT-based bipartite KS-uncolorability, we verify and extend the input counts of Trandafir and Cabello for bipartite perfect quantum strategies across all six islands. Cabello--Severini--Winter analysis shows that the algebraic substrate also determines the strength of quantum contextual advantage.
\end{abstract}
...
In this paper, we ask a structural question: \emph{what algebraic invariant controls whether a number field supports KS sets in dimension 3?} Through a systematic computational survey of quadratic fields $\Q(\sqrt{d})$ for $d = 2, \ldots, 30$, all nine imaginary quadratic fields with class number 1, cyclotomic fields, and golden ratio extensions, we find a sharp answer: KS-uncolorability requires the ring-of-integers generator to have \emph{norm at most 2}. This single invariant---the squared modulus of the generator in the ambient inner product---explains why constructions cluster into six discrete ``algebraic islands'' among the fields tested, separated by an algebraic barrier that no amount of additional vectors can overcome.

The norm-2 boundary unifies the known constructions: the integer identity $1 + 1 = 2$, the Peres identity $(\sqrt{2})^2 = 2$, the Eisenstein identity $1 + \omega + \omega^2 = 0$ (norm 1), and the Heegner-7 identity $\alpha\bar\alpha = 2$ are all manifestations of the same constraint---the coordinate alphabet must support cancellation identities that create exact orthogonalities among triples of vectors. At generator norm $\geq 3$, such cancellations become impossible. Our main contributions are:

\begin{enumerate}[label=(\roman*)]
    \item The identification of generator norm $\leq 2$ as the controlling invariant for KS-uncolorability across quadratic and cyclotomic fields (Sections~\ref{sec:quadratic}--\ref{sec:heegner}), connecting KS theory to algebraic number theory.
    \item Two new KS configurations: the Heegner-7 ring $\Z[(1+\sqrt{-7})/2]$ (43 vectors, a genuinely new orthogonality graph) and the golden ratio field $\Q(\varphi)$ (52 vectors, revealed only by cross-product completion). Neither has been previously reported (Sections~\ref{sec:heegner}, \ref{sec:golden}; Appendix~\ref{app:rays}).
    ...
    \item Supporting results: strong evidence that 31 is optimal for the integer island (Section~\ref{sec:exhaustive}), the pattern $6 \mid n$ for cyclotomic uncolorability (Section~\ref{sec:roots}), and graph isomorphism across the norm-2 family (Proposition~\ref{prop:isomorphism}).
\end{enumerate}
...
The six known algebraic islands arise from two fundamentally different cancellation mechanisms:
...
The \textbf{golden ratio island} ($\varphi^2 = \varphi + 1$, min 52) requires special comment. The generator $\varphi$ has norm $|\varphi|^2 \approx 2.618 > 2$, which would place it outside the norm-2 boundary. However, cross-product completion generates the reciprocal $1/\varphi$ (with $|1/\varphi|^2 < 1$), and the completed coordinate set contains the ratio $\varphi : 1/\varphi = \varphi^2 = \varphi + 1$, reintroducing norm-2-type cancellations indirectly. The golden island therefore does not violate the norm-2 boundary but rather demonstrates that the relevant criterion is not the generator's own norm but whether the \emph{effective coordinate set after completion} admits norm-2-type or phase-type cancellations.
...
\subsection{Completeness for two-element alphabets}\label{sec:completeness}
...
\toprule
Island & Alphabet $\mathcal{A}$ & Cancellation identity & Dot-product example & Type & Min \\
\midrule
...
Golden & $\{0,\pm 1,\pm\varphi\}$ $\to$ completed & $\varphi^2 = \varphi + 1$ & $1{\cdot}1 + 1{\cdot}\varphi + ({-}1){\cdot}\varphi^2 = 0$ & Indirect & 52 \\
\bottomrule
\end{tabular}
\end{table*}

We can show that this classification is \emph{complete} for two-element alphabets.  Consider an arbitrary alphabet $\{0, \pm 1, \pm x\}$ with $x \notin \{0, \pm 1\}$.  The Hermitian product set $\bar{\mathcal{A}} \cdot \mathcal{A}$ consists of $\{0, \pm 1, \pm x, \pm \bar{x}, \pm |x|^2\}$ (with $\bar{x} = x$ in the real case).  A non-trivial orthogonality $\sum_{k=1}^3 \bar{u}_k v_k = 0$ requires three elements of this product set to sum to zero, with at least one involving $x$ (otherwise the identity is trivial).  Table~\ref{tab:two-element} enumerates all such zero-sums.

\begin{table}[h]
\centering
\caption{All non-trivial three-term zero-sums from the product set of a two-element alphabet $\{0, \pm 1, \pm x\}$. Each row is an algebraic constraint on~$x$; every solution reduces to a known island.}
...
$1 + 1 - |x|^2 = 0$ & $|x|^2 = 2$ & $x = \sqrt{2}, \sqrt{-2}, \frac{1{+}\sqrt{-7}}{2}, \ldots$ & Peres / $\Z[\sqrt{-2}]$ / Heegner-7 \\
$1 + x - |x|^2 = 0$ & $|x|^2 = 1 + x$ & $x = \varphi$ (real) & Golden \\
$1 + x + |x|^2\mkern-2mu\cdot\mkern-2mu x = 0$ & $x^2 + x + 1 = 0$ & $x = \omega = e^{2\pi i/3}$ & Eisenstein \\
...
For any two-element alphabet $\{0, \pm 1, \pm x\}$, every non-trivial cancellation identity reduces to one of the six known islands (up to scaling equivalence).  The algebraic classification is therefore \textbf{complete} for two-element alphabets: no undiscovered island can arise from this family.
\end{observation}
...
This conjecture is supported by: the rigidity of $\CK$~\cite{TrandafirCabello2025} (uniqueness suggests optimality); exhaustive proof that 31 is minimum for the integer island (Proposition~\ref{prop:31optimal}); the absence of any sub-31 KS set across all six algebraic islands (Observation~\ref{obs:islands}); the complete failure of 30{,}000+ random hypergraphs to achieve geometric realizability; and the graph universality result below.
...
\begin{enumerate}
    ...
    \begin{itemize}
        ...
        \item \emph{Class-number $> 1$ imaginary quadratic fields.}  Our survey tested all nine class-1 fields (Heegner numbers) exhaustively, but rings with non-unique factorization---such as $\Z[\sqrt{-5}]$ (class number~2), where $2 \cdot 3 = (1+\sqrt{-5})(1-\sqrt{-5})$---could support cross-cancellation mechanisms unavailable in UFDs.  The multiple factorization paths create algebraic relations among ring elements that have no class-1 analogue.
        \item \emph{Simultaneous conjugate cancellations.}  Multi-element alphabets $\{0, \pm 1, \pm\alpha, \pm\bar\alpha, \pm(\alpha + \bar\alpha)\}$ where $\alpha$ and $\bar\alpha$ participate in \emph{independent} cancellation identities could produce richer orthogonality structures than single-generator alphabets.  The Gaussian case $\{0, \pm 1, \pm i, \pm(1+i)\}$ already exhibits this (both $|i|^2 = 1$ and $|1+i|^2 = 2$), but higher-degree fields offer more conjugate pairs.
...
\begin{enumerate}
    ...
    \item \textbf{Supporting evidence.} The $6 \mid n$ pattern for cyclotomic uncolorability connects to the $\Z[1/6]$ minimality result of Cortez, Morales, and Reyes. Strong computational evidence supports 31 as optimal for the integer island (Proposition~\ref{prop:31optimal}). A trigonometric consistency check across 164 angles finds only known islands, with KS-uncolorability at isolated algebraic points (Remark~\ref{rem:trig}). Most strikingly, the graph universality result (Observation~\ref{obs:universality}) shows that every construction achieving 31 rays---integers, rationals, mixed-field alphabets, and finite rotation group orbits---produces the \emph{same} orthogonality graph, suggesting that $\CK$ is the unique minimal realizable KS hypergraph in dimension~3.
\end{enumerate}

Whether the norm-2 boundary extends to all number fields, or whether higher-degree extensions harbor additional islands, remains the principal open question. The identification of Cabello's ``simplest KS set'' with the Eisenstein island suggests that the algebraic perspective may complement the ongoing search for optimal constructions. More concretely, the constraints established here---alphabet completeness, density monotonicity, and the discrete 13$\to$49 ray jump---are directly applicable to the realizability-based lower-bound program of Li, Bright, and Ganesh~\cite{LiBrightGanesh2024}: algebraic pruning of candidate hypergraphs could substantially narrow the search space that their SMT solver must explore, bringing the constructive upper bound (31) and the realizability lower bound (24) closer together.

\subsection*{Reproducibility}

All code and data are available at \url{https://github.com/michaelkernaghan/contextuality}. The repository includes explicit ray coordinates and triad/basis lists for all islands, including the 43-vector Heegner-7 set and the 52-vector golden-ratio set, as well as verifier scripts that reconstruct each KS set from its alphabet and confirm UNSAT via SAT solving. The implementation uses Python 3.11 with PySAT 1.8 (Glucose4 solver), NumPy, SciPy (L-BFGS-B optimizer and HiGHS LP solver), and CVXPY (SCS semidefinite programming solver, tolerance $10^{-9}$). Tables~\ref{tab:alphabets}--\ref{tab:closure} are generated by \texttt{ks\_islands.py}; Table~\ref{tab:roots} by \texttt{ks\_complex.py}; Table~\ref{tab:complex-quadratic} and the exhaustive 31-optimality check by \texttt{ks\_new\_islands.py} and \texttt{ks\_sat.py}; Table~\ref{tab:heegner} by \texttt{ks\_new\_island\_analysis.py}; Tables~\ref{tab:csw-min}--\ref{tab:csw-pool} and~\ref{tab:spectral} by \texttt{ks\_csw.py}, \texttt{ks\_csw\_extended.py}, and \texttt{ks\_graph\_analysis.py}; Table~\ref{tab:critical-bases} by \texttt{ks\_critical\_bases.py}; Table~\ref{tab:bpqs} by \texttt{ks\_bpqs\_sat.py}; the trigonometric sweep (Remark~\ref{rem:trig}) by \texttt{ks\_trig.py}; Proposition~\ref{prop:isomorphism} by \texttt{ks\_graph\_analysis.py}; Observation~\ref{obs:universality} by \texttt{ks\_group\_isomorphism.py}; and the cubic field, mixed alphabet, and group orbit explorations by \texttt{ks\_explore\_new.py} and \texttt{ks\_explore\_new2.py}. All randomized experiments use \texttt{random.seed(42)} for reproducibility.
