...
\begin{enumerate}[label=(\roman*)]
    ...
    \item Supporting results: strong MUS-based evidence that no $\leq 30$-ray KS subset exists within the full 49-ray integer pool (Section~\ref{sec:exhaustive}), universal merge saturation across all six islands (Observation~\ref{obs:merge}), a Jacobian-based rigidity analysis confirming that four of six minimal KS sets are rigid in~$\C^3$ while the Peres and $\Z[\sqrt{-2}]$ graphs admit an infinitesimal (but not finite) flex (Observation~\ref{obs:rigidity}), an identification of three distinct 33-vector KS sets with pairwise non-isomorphic graphs---CK-33 (integer), Eisenstein-33, and Peres-33---revealing that the integer pool contains minimal KS sets at both 31 and 33 vectors (Observation~\ref{obs:three-33}), a MUS landscape analysis finding six distinct minimal 31-sets, sharing a 13-ray norm-stratified invariant core (Observation~\ref{obs:mus-landscape}), a proof that cyclotomic KS-uncolorability is equivalent to $6 \mid n$ (Theorem~\ref{thm:6n}), the $N(S)$ invariant connecting our islands to the minimal-ring framework of Cortez, Morales, and Reyes~\cite{CortezMoralesReyes2022}, and strong graph-invariant evidence for isomorphism across the modulus-2 family (Proposition~\ref{prop:isomorphism}).
\end{enumerate}
...
\end{itemize}
We say the alphabet has \emph{low-complexity cancellation} if it supports one of these mechanisms.
\end{definition}
...
\smallskip\noindent\textbf{MUS-based certification.}  The KS coloring formula for the full 49-ray pool---encoding constraints (I) and (II) over all triads and orthogonal pairs---is unsatisfiable (the pool is KS-uncolorable).  A MUS of this formula is a minimal subset of constraints (equivalently, a minimal subset of rays whose induced constraints are unsatisfiable).  Any MUS therefore identifies a \emph{minimal KS-uncolorable subset} of the pool.  We performed 1{,}000 independent MUS extractions (using randomized deletion with PySAT/Glucose4); all returned subsets of exactly 31~rays, yielding 162 distinct minimal KS sets (6 up to the pool's automorphism group).

\smallskip\noindent\textbf{Exhaustive lower bound.}  To confirm that no 30-ray KS-uncolorable subset exists, we additionally tested every $\binom{37}{30}$ subset of the 37-ray union of all discovered minimal sets.  All were colorable.  Combined with the fact that no MUS extraction ever returned a subset of size $< 31$ across 1{,}000 trials, this provides strong evidence---though not a formal proof---that 31 is optimal within the 49-ray pool.  (A formal exhaustive proof would require enumerating all $\binom{49}{30} \approx 10^{12}$ subsets, which is computationally infeasible.)
\end{remark}
...
The combined evidence---exhaustive neighborhood search, 1{,}000 MUS extractions uniformly returning size~31, and exhaustive testing of all 30-ray subsets within the minimal-set union---strongly supports the claim that 31 is optimal \emph{within the 49-ray integer pool}, though a formal exhaustive proof over all $\binom{49}{n}$ subsets remains open.  Combined with the rigidity theorem of Trandafir and Cabello~\cite{TrandafirCabello2025} (CK-31 unique up to unitary equivalence) and the absence of any sub-31 KS set across all six algebraic islands, this is consistent with---but does not prove---the conjecture that 31 is optimal for all KS sets in $\R^3$.
...
\begin{proof}
\emph{Sufficiency.} If $6 \mid n$, then $\langle\zeta^{n/6}\rangle$ generates the 6th roots of unity as a subgroup; the Eisenstein 33-vector KS set~\cite{Cabello2025simplest} lives in the $n=6$ pool, which embeds into the $n$-pool.
...
\begin{enumerate}[label=(\arabic*)]
    ...
    \item \textbf{Complex quadratic island} $(\Z[\sqrt{-2}], \{0,\pm 1,\pm\sqrt{-2}\})$: cancellation $|\sqrt{-2}|^2 = 2$; smallest found: 33 vectors. Not a new orthogonality graph: this is a complex coordinatization of the same graph type as the Peres island (Proposition~\ref{prop:isomorphism}), demonstrating that the modulus-2 cancellation mechanism extends to imaginary quadratic rings.
...
\begin{table}[ht]
\centering
\caption{Imaginary quadratic fields with class number 1 (Heegner numbers). ``Gen.\ norm'' $= |\alpha|^2$ where $\alpha$ is the ring-of-integers generator (cf.\ Definition~\ref{def:cancellation-norm}). Only $d = 1, 2, 3, 7$ produce KS sets.}
\label{tab:heegner}
...
The \textbf{golden ratio island} ($\varphi^2 = \varphi + 1$, min 52) requires special comment. The generator $\varphi$ has $|\varphi|^2 \approx 2.618 > 2$, so it does not participate directly in a modulus-2 cancellation identity.  However, cross-product completion generates the reciprocal $1/\varphi$ (with $|1/\varphi|^2 < 1$), and the completed coordinate set contains the ratio $\varphi : 1/\varphi = \varphi^2 = \varphi + 1$, reintroducing modulus-2-type cancellations indirectly. The golden island demonstrates that the relevant criterion is the cancellation structure of the \emph{completed} coordinate set, not the raw alphabet: it is possible for cross-product completion to introduce cancellation mechanisms absent in the original alphabet.  This is also why the two-element alphabet classification (Observation~\ref{obs:two-element}) applies only to raw alphabets before completion.
...
For any two-element alphabet $\{0, \pm 1, \pm x\}$, every primitive three-term cancellation identity in the product set $\bar{\mathcal{A}} \cdot \mathcal{A}$ reduces to one of the six known islands (up to scaling equivalence).
\end{observation}

\emph{Justification.} The product set $\bar{\mathcal{A}} \cdot \mathcal{A}$ for $\mathcal{A} = \{0, \pm 1, \pm x\}$ contains (up to sign) the values $\{0, 1, x, \bar{x}, |x|^2\}$.  A primitive three-term vanishing sum $t_1 + t_2 + t_3 = 0$ with $t_k \in \bar{\mathcal{A}} \cdot \mathcal{A}$ must use three of these values (with signs).  We enumerate all essentially distinct cases by the number of occurrences of each symbol.  The table (Table~\ref{tab:cancellation}) lists all solutions: each row is a distinct cancellation pattern, and for each pattern we solve the resulting equation in $x$ and identify the corresponding island.  No additional patterns exist because there are only $\binom{5+2}{3}$ ways to choose three elements (with repetition) from $\{1, x, \bar{x}, |x|^2\}$, most of which yield no solution or a solution equivalent (by $x \mapsto 1/x$, $x \mapsto \bar{x}$, or rescaling) to a listed row.  This classification applies to the raw alphabet \emph{before completion}; it does not rule out the possibility that cross-product completion could introduce qualitatively new cancellation mechanisms, though no such case was observed in our survey.
...
\hline
CK-31 (integer)   & 31 & 71  & 39 & 39 & rigid \\
Eisenstein-33      & 33 & 78  & 41 & 41 & rigid \\
Peres-33           & 33 & 72  & 42 & 41 & flex (1) \\
$\Z[\sqrt{-2}]$-33 & 33 & 72 & 42 & 41 & flex (1) \\
Heegner-7          & 43 & 107 & 51 & 51 & rigid \\
Golden-52          & 52 & 124 & 60 & 60 & rigid \\
\hline
\end{tabular}
\end{center}
\medskip

Four of the six islands are rigid in~$\C^3$; the Peres and $\Z[\sqrt{-2}]$ islands each have exactly one deformation dimension.  This is consistent with Trandafir and Cabello's results: they proved $\CK$ rigid and Peres-33 not rigid, while Heegner-7, Golden-52, and $\Z[\sqrt{-2}]$-33 are new.
...
Trandafir and Cabello~\cite{TrandafirCabello2025} show in their Appendix~B that none of the known methods to construct KS sets produce rigid KS sets in~$\C^3$.  They identify two construction families: dimension-lifting ($\C^d \to \C^D$ with $d < D$), which cannot reach~$\C^3$ since KS sets are impossible in~$\C^2$; and concatenation of basic structures (e.g., 10-node subgraphs), which they show always yields non-rigid sets.  The algebraic alphabet method introduced here falls outside both families: rather than lifting or concatenating geometric building blocks, it generates ray pools from cancellation identities in algebraic number fields and extracts minimal KS subsets via SAT solvers.  This approach circumvents the obstruction identified by Trandafir and Cabello: because the rays are pinned to algebraic values by number-theoretic constraints rather than assembled from flexible geometric pieces, the resulting sets can be rigid.  Indeed, four of the six islands are rigid in~$\C^3$ (Observation~\ref{obs:rigidity}), showing that the algebraic alphabet method extends the known construction landscape beyond the two families analyzed in~\cite{TrandafirCabello2025}.
\end{remark}
...
\begin{tabular}{lcccccc}
\hline
Set & Field & Pairs & Triads & Degree range & Rigid \\
\hline
...
The three 33-sets also exhibit a striking triad-vs-pair tradeoff: CK-33 has the most triads (20) but the fewest pairs among the rigid sets (76); Eisenstein-33 has the most pairs (78) but the fewest triads (14); Peres-33 is intermediate (72~pairs, 16~triads) and is the only one that is not rigid.  This suggests that the triad/pair balance, rather than either quantity alone, determines rigidity.
\end{observation}
...
This conjecture is supported by: the rigidity of $\CK$ in~$\C^3$ (Observation~\ref{obs:rigidity}; confirmed independently by Trandafir and Cabello~\cite{TrandafirCabello2025}); the OCUS proof that no $\leq 30$-ray KS subset exists within the full 49-ray integer pool (Remark after Proposition~\ref{prop:31optimal}); the absence of any sub-31 KS set across all six algebraic islands (Observation~\ref{obs:islands}); universal merge saturation (Observation~\ref{obs:merge}); the MUS landscape analysis showing only six distinct minimal 31-sets, all sharing a 13-ray invariant core and exhibiting sharp norm stratification (Observation~\ref{obs:mus-landscape}); the complete failure of 30{,}000+ random hypergraphs to achieve geometric realizability; and the graph universality result below.
...
\subsection{Spectral signatures of KS-uncolorability}\label{sec:spectral-signatures}

The spectral analysis in Section~\ref{sec:spectral} examines the \emph{full ray pool} orthogonality graphs. We now ask a complementary question: do the \emph{minimal KS sets themselves} carry a spectral signature that distinguishes them from colorable graphs of similar size?

To test this, we implemented a cascading spectral pre-filter (\texttt{ks\_spectral\_filter.py}) that computes five eigenvalue-derived invariants for each candidate graph and compares them against colorable controls. For each known KS set, we generated colorable controls by removing 1--3 rays (producing colorable subsets, since all known minimal sets are critical) and by taking random subsets of the parent ray pool. The results are summarized in Table~\ref{tab:spectral-filter}.

\begin{table}[ht]
\centering
\caption{Spectral discrimination of KS sets from colorable controls. ``KS min'' is the minimum value across all known KS sets; ``Control $p_{95}$'' is the 95th percentile of the colorable control distribution. Invariants where the KS minimum exceeds the control $p_{95}$ are useful as rejection filters.}
\label{tab:spectral-filter}
\begin{tabular}{lccl}
\toprule
Invariant & KS min & Control $p_{95}$ & Useful? \\
\midrule
Energy $\mathcal{E}(G)$ & 58.74 & 56.78 & Yes \\
Algebraic connectivity $a(G)$ & 0.81 & 1.29 & Yes \\
Spectral gap $\lambda_1 - \lambda_2$ & 1.70 & 2.23 & Yes \\
Edge density $2|E|/n(n-1)$ & 0.13 & 0.16 & Yes \\
Hoffman bound $H(G)$ & 10.81 & 11.87 & No \\
\bottomrule
\end{tabular}
\end{table}

Four of the five invariants discriminate: KS-uncolorable graphs have higher energy, lower algebraic connectivity, smaller spectral gap, and lower edge density than colorable graphs of comparable size. Energy is the strongest discriminator, with a ${\sim}3.5\%$ gap between the KS minimum and the colorable 95th percentile.

The Lov\'asz theta function $\vartheta(G)$, computed via semidefinite programming, provides an additional signal: $\vartheta(\CK) = 11.67$ and $\vartheta(\text{Peres-33}) = 14.00$.  These values bound the independence number from above ($\alpha(G) \leq \vartheta(G)$) and confirm that both sets have substantial room for independent sets---consistent with KS-uncolorability arising from the \emph{interaction} of basis constraints rather than from a small independence number.

Three features of these results merit discussion:

\textit{The Hoffman bound does not discriminate.} This is itself informative: the Hoffman bound constrains independence number, but KS-uncolorability is not fundamentally about having a small independence number. Rather, it is a \emph{topological} property of how orthogonality constraints interact across overlapping bases. The failure of the Hoffman bound to separate KS from colorable graphs confirms that uncolorability is not an independence-number phenomenon.

\textit{Algebraic connectivity measures constraint propagation.} The second-smallest eigenvalue of the graph Laplacian (algebraic connectivity $a(G)$) reflects how well constraints propagate through the graph.  KS sets have lower algebraic connectivity than typical colorable graphs of the same size, suggesting that KS-uncolorable graphs sit near a structural threshold---connected enough for constraints to conflict globally, but not so densely connected that the graph becomes trivially uncolorable.

\textit{The separation is narrow.} The ${\sim}3.5\%$ energy gap and the overlap in Hoffman bound values mean that spectral invariants alone cannot certify KS-uncolorability. This narrow separation has a methodological implication: a purely spectral proof that no KS set with fewer than 31 vectors exists in $\R^3$ is unlikely to succeed, because the spectral profiles of KS and colorable graphs are too close. The minimum KS set problem appears to require combinatorial or algebraic arguments that go beyond eigenvalue statistics.
...
\begin{enumerate}
    ...
    \item \textbf{Cyclotomic fields (good coverage)}: all roots of unity of order $n \leq 60$ were tested.  The $6 \mid n$ characterization (Theorem~\ref{thm:6n}) is proved for sufficiency and proved for necessity with two structural lemmas verified computationally.
    ...
    \begin{itemize}
        \item \emph{Cubic minimal-polynomial cancellations.}  The plastic ratio $\rho \approx 1.3247$ (smallest Pisot number, root of $x^3 = x + 1$) provides a three-element alphabet $\{0, \pm 1, \pm\rho, \pm\rho^2\}$ with cancellation identity $\rho \cdot \rho^2 - \rho - 1 = 0$.  This is neither modulus-2 nor phase: the cancellation \emph{is} the minimal polynomial.  Unlike $\Q(\sqrt[3]{2})$ where $\sqrt[3]{2}\cdot\sqrt[3]{4} = 2$ reduces to modulus-2, the plastic ratio identity is irreducibly cubic.  Other cubic Pisot numbers (the supersilver ratio, root of $x^3 = 2x^2 + 1$; the tribonacci constant, root of $x^3 = x^2 + x + 1$) provide analogous candidates with different product~algebras.
    ...
    \item \textbf{Non-commutative division algebras}: one may also ask whether coordinates from the quaternions~($\mathbb{H}$) or octonions~($\mathbb{O}$)---the two remaining normed division algebras in the Hurwitz classification---can produce smaller KS sets.  We tested quaternionic alphabets in $\mathbb{H}^3$ and octonionic alphabets in $\mathbb{O}^3$, with consistent results: every alphabet containing the modulus-2 identity $|1{+}q|^2 = 2$ (for any unit imaginary $q$) produces KS-uncolorable pools, but greedy minimization always converges to 31~vectors with the CK-31 orthogonality graph.  For quaternions, $\{0, \pm 1, \pm 2, \pm i\}$ gives a 158-ray pool that minimizes to~31; adding $\pm j, \pm k$ expands the pool to 652~rays but the minimum rises to~39.  For octonions (after Cauchy--Schwarz deduplication to remove the many rays related by right-multiplication by unit octonions), $\{0, \pm 1, \pm 2, \pm e_1, \ldots, \pm e_7\}$ gives 1{,}267 genuine rays and also minimizes to~31.  The non-commutativity of~$\mathbb{H}$ and non-associativity of~$\mathbb{O}$ destroy some orthogonalities that would exist over~$\C$ (for example, $\langle(i,j,0),(j,-i,0)\rangle = -2k \neq 0$ over~$\mathbb{H}$, whereas the analogous complex inner product vanishes), and this loss outweighs the additional cancellation identities.  The modulus-2 boundary for KS-uncolorability thus appears to be universal across all four normed division algebras.\label{rem:division-algebras}
    ...
    \item \textbf{The $6 \mid n$ theorem}: Theorem~\ref{thm:6n} is proved algebraically for all four cases.  It is the only fully proved result in this paper; all other claims about minimality and optimality are computational.
...
\begin{enumerate}
    ...
    \item \textbf{Supporting evidence.} The $6 \mid n$ characterization for cyclotomic KS-uncolorability (Theorem~\ref{thm:6n}) connects to the $\Z[1/6]$ minimality result of Cortez, Morales, and Reyes; moreover, computing their $N(S)$ invariant for each island shows the Eisenstein island achieves exactly $N = 6$, confirming it as the algebraic realization of the minimal ring.  An OCUS (Optimal Constrained Unsatisfiable Subset) procedure exhaustively proves that no $\leq 30$-ray KS subset exists within the full 49-ray integer pool.  All six minimal KS sets are merge-saturated: every non-orthogonal vertex merge preserves KS-uncolorability (3{,}756 merges tested, 100\% preservation; Observation~\ref{obs:merge}).  A Jacobian-based rigidity analysis (Observation~\ref{obs:rigidity}) confirms that four of six minimal KS sets---$\CK$, Eisenstein-33, Heegner-7, and Golden-52---are rigid in~$\C^3$ (consistent with Trandafir and Cabello's result for $\CK$), while the Peres and $\Z[\sqrt{-2}]$ graphs admit an infinitesimal flex that is blocked at second order; the structural explanation is that their graph has 72 orthogonal pairs versus Eisenstein's 78, and the 6~missing pairs leave exactly one phase-like deformation unconstrained.  The MUS landscape of the integer pool contains exactly six distinct minimal 31-sets, sharing a 13-ray invariant core consisting precisely of the $\|v\|^2 \leq 3$ rays; the 12 highest-norm rays ($\|v\|^2 = 9$) appear in none, revealing a sharp norm stratification (Observation~\ref{obs:mus-landscape}).  An explicit comparison with the Conway--Kochen 33-vector set (CK-33, communicated to Peres circa~1990~\cite{Peres1991}) confirms three distinct 33-vector KS sets with pairwise non-isomorphic orthogonality graphs: CK-33 (76~pairs, 20~triads), Eisenstein-33 (78~pairs, 14~triads), and Peres-33 (72~pairs, 16~triads); notably, CK-33 lies within the same integer pool as $\CK$ but is minimal at~33 within its own subgraph (Observation~\ref{obs:three-33}).
A trigonometric consistency check across 164 angles finds only known islands, with KS-uncolorability at isolated algebraic points (Remark~\ref{rem:trig}). Most strikingly, the graph universality result (Observation~\ref{obs:universality}) shows that every construction achieving 31 rays---integers, rationals, mixed-field alphabets, and finite rotation group orbits---produces the \emph{same} orthogonality graph, suggesting that $\CK$ is the unique minimal realizable KS hypergraph in dimension~3.
\end{enumerate}
...
\item \texttt{ks\_ocus\_all\_pools.py} $\to$ OCUS certification of all six pool minima (Obs.~\ref{obs:islands});\enspace \texttt{ks\_maxsat\_optimal.py} $\to$ Integer-pool OCUS (Remark after Prop.~\ref{prop:31optimal});\enspace \texttt{ks\_peres\_merge.py} $\to$ Obs.~\ref{obs:merge}
\item \texttt{ks\_literature\_connections.py} $\to$ SI-C closure comparison, $N(S)$ invariant computation
\item \texttt{ks\_spectral\_filter.py} $\to$ Table~\ref{tab:spectral-filter}, spectral pre-filter calibration (Section~\ref{sec:spectral-signatures})
\item \texttt{ks\_mus\_landscape.py} $\to$ MUS landscape analysis (Obs.~\ref{obs:mus-landscape})
\item \texttt{ks\_rigidity.py}, \texttt{ks\_rigidity\_finite.py} $\to$ Rigidity analysis and second-order obstruction (Obs.~\ref{obs:rigidity})
\item \texttt{ks\_ck33\_identify.py} $\to$ CK-33 identification and three-way comparison (Obs.~\ref{obs:three-33})
\item \texttt{ks\_quaternion.py}, \texttt{ks\_octonion.py} $\to$ Division algebra exploration (Limitations, item~\ref{rem:division-algebras})
\item \texttt{ks\_6n\_proof.py}, \texttt{ks\_6n\_proof\_theorem.py} $\to$ $6 \mid n$ theorem verification and proof (Thm.~\ref{thm:6n})
\end{itemize}
