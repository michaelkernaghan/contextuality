...
\textbf{Vanishing-sum lemma.}  A three-term sum $\zeta^a + \zeta^b + \zeta^c = 0$ with $\zeta = e^{2\pi i/n}$ exists if and only if $3 \mid n$.  \emph{Proof}: dividing by $\zeta^a$ gives $1 + \zeta^p + \zeta^q = 0$; setting $\alpha = \zeta^p$ requires $|-1-\alpha| = 1$, which forces $\cos(2\pi p/n) = -1/2$, hence $p/n \in \{1/3, 2/3\}$, so $3 \mid n$.  Similarly, $\zeta^a + \zeta^b = 0$ requires $\zeta^{b-a} = -1$, hence $2 \mid n$.

\textbf{Case 1: $3 \nmid n$.}  No three-term vanishing sum exists, so all triads must involve rays with zero coordinates and at most 2-term cancellations.  The resulting triad network is too sparse for KS-uncolorability.  (Verified for all $n \leq 60$.)

\textbf{Case 2: $3 \mid n$, $2 \nmid n$.}  The three-term cancellation $1 + \omega + \omega^2 = 0$ creates triads, but since $-1$ is not an $n$th root of unity when $n$ is odd, no two-term cancellation exists.  The key structural consequence is that \emph{every triad is isolated}: no two triads share a ray.  This is because the phase constraint $\zeta^a + \zeta^b + \zeta^c = 0$ has the unique solution (up to permutation and common phase) with exponent differences $n/3$ and $2n/3$; this rigidity means each ray's triad partners are uniquely determined.  With all triads isolated, the coloring problem decomposes into $|T|$ independent subproblems, each trivially satisfiable by assigning one green ray per triad.  (Verified for all odd $3 \mid n$ up to $n = 63$; triad isolation confirmed for all tested cases.)
\end{proof}
...
\begin{enumerate}
    ...
    \item \textbf{The $6 \mid n$ theorem}: Theorem~\ref{thm:6n} provides a proof sketch whose key lemmas (vanishing-sum characterization, triad isolation for odd $3 \mid n$) are proved algebraically and verified computationally up to $n = 63$.  A fully formal proof for all $n$ would require formalizing the triad decomposition structure of Case~1 ($3 \nmid n$, $2 \mid n$).
