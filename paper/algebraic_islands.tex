...
We quantify the ``size'' of a cancellation identity through the squared moduli of its terms.

\begin{definition}\label{def:cancellation-norm}
Let $\mathcal{A} = \{0, \pm 1, \pm x\}$ be a two-symbol coordinate alphabet.  The \emph{product set} $\bar{\mathcal{A}} \cdot \mathcal{A} = \{\bar{a}b : a, b \in \mathcal{A}\}$ contains all values that can appear as individual terms in a dot product $\langle v | w \rangle = \sum_k \bar{v}_k w_k$.  A \emph{cancellation identity} is a vanishing sum $\sum_{k} t_k = 0$ with $t_k \in \bar{\mathcal{A}} \cdot \mathcal{A}$ (at most three terms for $d = 3$).  The \emph{cancellation norm} of such an identity is $\max_k |t_k|^2$.  We say the alphabet has \emph{norm-2 cancellation} if it supports an identity with cancellation norm $\leq 2$, and \emph{phase cancellation} if it supports a vanishing sum of unit-modulus terms (all $|t_k| = 1$).
\end{definition}

For the six known KS-supporting alphabets, the cancellation norms are: integer ($1+1=2$, norm 2), Peres ($(\sqrt{2})^2=2$, norm 2), Eisenstein ($1+\omega+\omega^2=0$, norm 1), $\Z[\sqrt{-2}]$ ($|\sqrt{-2}|^2=2$, norm 2), Heegner-7 ($\alpha\bar\alpha=2$, norm 2), and golden ($\varphi^2=\varphi+1$, norm $\varphi^2 \approx 2.618$---but cross-product completion introduces $1/\varphi$ with $|1/\varphi|^2 < 1$, creating effective norm-2 cancellations in the completed pool; see Section~\ref{sec:golden}).  No alphabet with minimum cancellation norm $\geq 3$ produces a KS set in our survey.
...
\begin{remark}
The exhaustive check above covers the 37-ray union of discovered 31-sets, not the entire 49-ray pool (49 rays, 71 orthogonal pairs, 17 triads).  We close this gap using an OCUS (Optimal Constrained Unsatisfiable Subset) approach, a CEGAR loop that exhaustively proves no $n$-ray KS subset exists:

\smallskip\noindent\textbf{OCUS procedure.} For a target size $n$, introduce selector variables $b_v \in \{0,1\}$ for each of the 49 rays, with cardinality constraint $\sum_v b_v \leq n$.  The KS coloring variables $x_v$ (ray $v$ is colored~1) are conditioned on selection: if $b_v = 0$, then $x_v = 0$.  For each triad $\{i,j,k\}$, the exactly-one constraint applies only when all three rays are selected ($b_i = b_j = b_k = 1$); orthogonal exclusion applies only when both rays are selected.  The CEGAR loop proceeds:
\begin{enumerate}[label=(\arabic*),nosep]
\item Solve the SAT instance.  If UNSAT, output ``no $n$-ray KS subset exists'' (proved).
\item If SAT, the assignment $\{v : b_v = 1\}$ identifies a colorable $n$-ray subset.  Add a blocking clause excluding this exact selection and return to~(1).
\end{enumerate}
The loop terminates because each iteration eliminates at least one candidate subset from the finite space $\binom{49}{n}$.  For $n = 30$, the loop exhausted in 272 iterations ($< 0.1$\,s); all sizes $n = 21, \ldots, 30$ completed in total $< 0.5$\,s.  As independent confirmation, 1{,}000 MUS (Minimal Unsatisfiable Subset) extractions all returned subsets of size~31, yielding 162 distinct minimal KS sets.
\end{remark}
