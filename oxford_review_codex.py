"""Send algebraic_islands.tex to GPT-5.4-pro for Oxford-style challenge review."""
import requests
import re
import sys
...
# Read the paper
tex = Path(r"C:\Users\Michael Kernaghan\contextuality\paper\algebraic_islands.tex").read_text(encoding="utf-8")
...
prompt = """You are conducting an Oxford-style challenge review of a research paper in quantum foundations and algebraic combinatorics. The paper classifies Kochen-Specker sets in dimension 3 by algebraic number field, proposing that generator norm <= 2 is the controlling invariant.

Complete ALL THREE steps thoroughly:
...
# Save review FIRST (before printing, to avoid Windows encoding crash)
timestamp = datetime.now().strftime("%Y%m%d-%H%M%S")
outpath = Path(r"C:\Users\Michael Kernaghan\claude-inbox\peer-reviews") / f"algebraic_islands-oxford-gpt54-{timestamp}.txt"
outpath.parent.mkdir(parents=True, exist_ok=True)
with open(outpath, "w", encoding="utf-8") as f:
    f.write(f"Oxford Challenge Review: algebraic_islands.tex\n")
...
# Print review with safe encoding
